...
from application import (
    app_systemd_env_lines,
    app_container_problems,
    app_route_problems,
    app_secret_digests,
    app_secret_reconcile,
)
...
def test_digests_are_sha256_of_the_string_value():
    assert app_secret_digests({"a": "one"}) == {"a": digest("one")}


def test_digests_are_stable_across_calls():
    values = {"a": "one", "b": "two"}
    assert app_secret_digests(values) == app_secret_digests(values)


@pytest.mark.parametrize("value", ["plain", "with spaces", "100%", 8080, True, ""])
def test_digests_accept_any_scalar(value):
    assert app_secret_digests({"k": value}) == {"k": digest(value)}


def test_digests_of_nothing_is_empty():
    assert app_secret_digests(None) == {}
    assert app_secret_digests({}) == {}


# --- app_secret_reconcile --------------------------------------------------------------

VALUES = {"app-a": "one", "app-b": "two"}
DIGESTS = app_secret_digests(VALUES)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("rotated value", app_secret_digests({"app-a": "NEW", "app-b": "two"}), RECORD,
         ["app-a", "app-b"], ["app-a"], []),
        ("name new to the file", app_secret_digests({**VALUES, "app-c": "three"}), RECORD,
         ["app-a", "app-b"], ["app-c"], []),
        ("name dropped from the file", app_secret_digests({"app-a": "one"}), RECORD,
         ["app-a", "app-b"], [], ["app-b"]),
        ("first deploy, no record", DIGESTS, "", [], ["app-a", "app-b"], []),
        ("app with no secrets", {}, "", [], [], []),
        ("another app's secret in the store", DIGESTS, RECORD,
         ["app-a", "app-b", "other-x"], [], []),
        ("rotation and drift together", app_secret_digests({"app-a": "NEW", "app-b": "two"}),
         RECORD, ["app-a"], ["app-a", "app-b"], []),
        ("renamed: old dropped, new stored", app_secret_digests({"app-a": "one", "app-c": "two"}),
         RECORD, ["app-a", "app-b"], ["app-c"], ["app-b"]),
    ],
)
def test_reconcile(label, digests, record, stored, expect_store, expect_drop):
    got = app_secret_reconcile(digests, record, stored)
...
def test_remove_is_the_union_of_store_and_drop():
    ...
    got = app_secret_reconcile(
        app_secret_digests({"app-a": "NEW", "app-c": "three"}), RECORD, ["app-a", "app-b"]
    )
...
def test_reconcile_output_is_sorted():
    digests = app_secret_digests({"z": "1", "a": "1", "m": "1"})
    assert app_secret_reconcile(digests, "", [])["store"] == ["a", "m", "z"]


@pytest.mark.parametrize("record", ["", None])
def test_missing_record_stores_everything(record):
    assert app_secret_reconcile(DIGESTS, record, [])["store"] == ["app-a", "app-b"]
...
def test_empty_record_file_parses_and_stores_everything():
    ...
    got = app_secret_reconcile(DIGESTS, recorded({}), ["app-a", "app-b"])
...
def test_blank_record_file_parses_as_no_record():
    ...
    assert app_secret_reconcile(DIGESTS, base64.b64encode(b"  \n").decode("ascii"))["store"] \
        == ["app-a", "app-b"]
...
def test_corrupt_record_is_refused_rather_than_ignored(content):
    ...
    with pytest.raises(AnsibleFilterError):
        app_secret_reconcile(DIGESTS, content, [])
...
def test_good_domains_are_accepted(domain):
    assert app_route_problems(domain, "calculators", 8080) == []
...
def test_bad_domains_are_rejected(domain, why):
    problems = app_route_problems(domain, "calculators", 8080)
...
def test_good_upstreams_are_accepted(upstream):
    assert app_route_problems("x.example.com", upstream, 8080) == []


@pytest.mark.parametrize("upstream", ["calc alc", "-calc", ".calc", "calc/x", "", None])
def test_bad_upstreams_are_rejected(upstream):
    problems = app_route_problems("x.example.com", upstream, 8080)
...
def test_good_ports_are_accepted(port):
    assert app_route_problems("x.example.com", "calc", port) == []


@pytest.mark.parametrize("port", [0, -1, 65536, 70000, "http", "", None, "80 80"])
def test_bad_ports_are_rejected(port):
    problems = app_route_problems("x.example.com", "calc", port)
...
def test_every_problem_is_reported_at_once():
    ...
    assert len(app_route_problems("bad {", "-bad", 0)) == 3
...
def test_nothing_configured_is_no_problem():
    assert app_container_problems(None) == []
    assert app_container_problems({}) == []
...
def test_the_real_call_sites_validate():
    ...
    assert app_route_problems("calc.cloudyhome.org", "calculators", 8080) == []
    assert app_route_problems("nasplan.cloudyhome.org", "nasplan", 8080) == []
    assert app_route_problems("whichday.cloudyhome.org", "whichday", 8080) == []
    assert app_container_problems({"FORWARD_HEADERS_STRATEGY": "native"},
                                 "Calculators web app") == []
    assert app_container_problems({}, None) == []
...
def test_plain_value_is_quoted():
    assert app_systemd_env_lines({"A": "native"}) == ['Environment="A=native"']
